...
ソフトウェアの誕生には、ニーズの発生や、要件定義書の作成が含まれる。要件定義書とは、システムの目的や機能の説明、目標性能、他システムとのインターフェース仕様、開発スケジュールなどが記された文書である。企業によって要件定義項目は異なるが、実現する機能及び実現しない機能を明確にすることが要件定義において重要となる。
...
# 第8回、第9回、第10回
・第8回からの講義では、ソフトウェアのバージョン管理について学んだ。
...
gitは変更履歴の管理や過去への復元、複数人での安全な共同作業を実現する分散型のバージョン管理システムである。開発者はリポジトリと呼ばれるソフトウェアのソースコードのデータプールをネットワーク越しに複数人で共有しながら開発を行う。
gitでは、ネットワーク越しにあるリポジトリ(リモートリポジトリ)を、開発者ごとのローカル環境にコピーすることで、ネットワークから切り離した環境(ローカルリポジトリ)で開発を進めることができる。
...
# 第11回
・第11回の講義では、Githubについての学習と、Githubで簡易的なプロジェクトを作成する演習を行った。
githubとは、リモートリポジトリのホスティングサービスの一つであり、webサイトを通じて、複数の開発者間でのリポジトリの共有をより円滑に行うことを可能としている。
github上では他人のリモートリポジトリをforkを用いることで自分のリモートリポジトリへ複製することができ、cloneを用いることでローカルリポジトリへ複製できる。
githubのプロジェクトのサイト画面では、開発者間でリポジトリに関する問題報告や機能追加、質問を上げるための"issues"、現在進行している作業を開発者間で共有できる"projects"といったページがあり、円滑なチーム開発のための便利機能が備わっている。
演習では、
