...
PEP8はコードの長さやコメントの書き方、スペースの入れ方などにルールを定めたスタイルガイドである。講義では、任意のPythonファイルをPEP8に基づいて精査するツールであるflake8を使って、与えられたPythonファイルから検出されたスタイルエラーをすべて解決する課題が出された。

# 第8回、第9回、第10回
・第8回からの講義では、ソフトウェアのバージョン管理について学んだ。
ソフトウェアのバージョンを更新していくにあたって、いつ、誰が、どのような変更を加えたのかの情報を記録することや、更新後に予想外の不具合が発生した場合に更新前の状態に戻せることは不可欠である。これらの課題を解決してくれるバージョン管理のためのツールの1つにgitがある。
gitは変更履歴の管理や過去への復元、複数人での安全な共同作業を実現する分散型のバージョン管理システムである。開発者はリポジトリと呼ばれるソフトウェアのソースコードのデータプールをネットワーク越しに複数人で共有しながら開発を行う。gitでは、ネットワーク越しにあるリポジトリ(リモートリポジトリ)を、開発者ごとのローカル環境にコピーすることで、ネットワークから切り離した環境(ローカルリポジトリ)で開発を進めることができる。
